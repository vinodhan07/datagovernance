...
from models import AIModel, AIComplianceCheck
...
def register_model(data: dict, db: Session = Depends(get_db)):
    ...
    model = AIModel(
        ...
        uses_pii       = 1 if data.get("uses_pii") else 0,
        autonomous     = 1 if data.get("autonomous") else 0,
...
def update_model(model_id: int, data: dict, db: Session = Depends(get_db)):
    ...
    if "uses_pii" in data:
        model.uses_pii = 1 if data["uses_pii"] else 0
    if "autonomous" in data:
        model.autonomous = 1 if data["autonomous"] else 0
