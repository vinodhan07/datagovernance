...
from src.domain.entities import PipelineRun, Integration
from src.modules.registry.integrations_service import get_connection_config
from src.modules.nexus.spline_consumer_sync import (
    fetch_latest_event, fetch_execution_plan,
    build_lineage_data_from_plan, build_fallback_lineage_data,
)
from src.modules.nexus.lineage_persistence import persist_lineage
...
def get_capabilities(integration_id: str, db: Session = Depends(get_db)):
    ...
    """
    from src.modules.registry.preflight_detector import detect_mariadb_capabilities, detect_github_capabilities
    from src.modules.registry.integrations_service import get_connection_config
...
async def _mariadb_pipeline(integration_id: str, db: Session):
    ...
    try:
        ...
        try:
            from src.modules.guardian.quality_auto_rules import auto_generate_rules
        ...
        from src.modules.nexus.spark_etl import create_spark_session, run_table_etl
        ...
        try:
            from src.modules.registry.openmetadata_sync import push_table_metadata
            pushed = 0
            for table in tables:
...
async def _github_pipeline(integration_id: str, db: Session, integration: Integration):
    ...
    try:
        ...
        from src.modules.registry.preflight_detector import detect_github_capabilities
...
def _load_data_file(
    ...
    from src.modules.guardian.quality_auto_rules import build_schema_from_dataframe
